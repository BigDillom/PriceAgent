"""Phoenix (凤凰票据) autocallable product."""
...
@dataclass
class Phoenix(Product):
    """Phoenix note: monthly knock-out, conditional coupon above yield barrier, daily knock-in."""
    ...
    in_obs_type: ExerciseType = ExerciseType.AMERICAN
    ...
    def __post_init__(self) -> None:
        ...
        by = self.barrier_yield if self.barrier_yield is not None else self.barrier_in
        self._barrier_yield = np.full(n, float(by))
    ...
    def payoff(self, path_or_spot: np.ndarray | float) -> float | np.ndarray:
        raise NotImplementedError("Phoenix payoff is path-dependent; use MC engine")
